...
def get_movie_byid(movieid):
   response = requests.get(f"http://{HOST}:3200/movies/{movieid}")
   return make_response(response.json(), response.status_code)

@app.route("/titles", methods=['GET'])
def get_movie_bytitle():
   title = request.args.get('title')
   response = requests.get(f"http://{HOST}:3200/titles", params={'title': title})
   return make_response(response.json(), response.status_code)

@app.route("/movies/<movieid>", methods=['POST'])
def create_movie(movieid):
   req = request.get_json()
   response = requests.post(f"http://{HOST}:3200/movies/{movieid}", json=req)
   return make_response(response.json(), response.status_code)

@app.route("/movies/<movieid>", methods=['DELETE'])
def del_movie(movieid):
   response = requests.delete(f"http://{HOST}:3200/movies/{movieid}")
   return make_response(response.json(), response.status_code)

@app.route("/movies/<movieid>/<rate>", methods=['PUT'])
def update_movie_rating(movieid, rate):
   response = requests.put(f"http://{HOST}:3200/movies/{movieid}/{rate}")
   return make_response(response.json(), response.status_code)
