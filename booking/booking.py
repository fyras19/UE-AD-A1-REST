...
def add_booking(userid):
   ...
   booking_response = requests.get(f"http://172.20.70.175:3201/bookings/{userid}")  
   ...
   showtime = requests.get(f"http://172.20.70.175:3202/showmovies/{date}")
            ...
            if booking["userid"]==str(userid):
               ...
               for date_user in booking["dates"]:
                  if date_user["date"]==str(date):
                     ...
                     return make_response(jsonify({"booking":booking}),200)
